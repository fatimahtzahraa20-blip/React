...
In Vercel, choose **Add New → Project**, import that repository, and set **Root Directory** to **`Academic-project`**. This repository contains other projects, so do not use the repository root. The committed configuration selects Vite, runs `npm ci` and `npm run build`, and serves `dist`. It also handles direct SPA links.
...
If the Vercel project was previously deployed from the repository root, change Project Settings ? Build and Deployment ? Root Directory to `Academic-project`, then redeploy.
