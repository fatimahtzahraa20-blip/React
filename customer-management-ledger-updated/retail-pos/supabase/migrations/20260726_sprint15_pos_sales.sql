...
create index if not exists held_sales_created_at_idx on public.held_sales(created_at desc);
...
begin
  ...
  loop
    ...
    v_subtotal := v_subtotal + ((v_item->>'quantity')::numeric * (v_item->>'sale_price')::numeric);
  ...
  loop
    ...
    values (v_invoice_id, (v_item->>'product_id')::bigint, (v_item->>'quantity')::numeric, (v_item->>'sale_price')::numeric, coalesce((v_item->>'discount')::numeric,0), (v_item->>'quantity')::numeric * (v_item->>'sale_price')::numeric);
...
create policy "users_manage_own_held_sales" on public.held_sales
for all to authenticated using (held_by = auth.uid()) with check (held_by = auth.uid());
