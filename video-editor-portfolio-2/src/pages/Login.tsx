...
export default function Login() {
  ...
  const { user, profile, loading: authLoading, isAdmin, signIn } = useAuth();
      ...
      <section className="w-full max-w-md rounded-2xl border border-zinc-200 bg-white p-6 shadow-xl dark:border-zinc-800 dark:bg-zinc-900 sm:p-8">
        <div className="mb-8 text-center">
          ...
          <p className="mt-3 text-sm leading-6 text-zinc-600 dark:text-zinc-300">
            Admin accounts open the admin dashboard. Approved users open the
            user dashboard.
          </p>
        </div>
        ...
        <div className="mt-6 rounded-lg border border-zinc-200 bg-zinc-50 p-4 text-sm text-zinc-600 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-300">
          <p>
            <span className="font-semibold">Admin:</span>{" "}
            the profile role must be set to admin.
          </p>

          <p className="mt-2">
            <span className="font-semibold">User:</span>{" "}
            the account must be approved and active.
          </p>
        </div>
